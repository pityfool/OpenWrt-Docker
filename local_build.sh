...
# 确保只有一个 tar.gz 文件用于构建，或者 Dockerfile 会添加所有
# 为了安全起见，我们应该清理旧的 tar.gz
# 但这里我们假设用户知道自己在做什么，或者我们创建一个临时目录来构建 Docker 镜像

TAG_NAME="openwrt-$BUILD_TYPE:$DEVICE_PLATFORM"

# 注意：Dockerfile 使用 ADD *.tar.gz /
# 如果目录下有多个 tar.gz 可能会有问题。
# 建议清理其他 tar.gz 或者创建一个 build context 目录。
# 简单起见，我们直接构建，但提示用户注意。
